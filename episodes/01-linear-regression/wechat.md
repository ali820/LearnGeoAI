...
图1 高程与温度的散点关系图。示例数据为教学构造数据，用于展示变量之间是否存在近似线性趋势，不代表真实观测。
...
图2 高程与温度的线性拟合图。红色直线表示模型估计的平均关系，斜率用于描述高程变化时温度的平均变化方向和幅度。
...
图3 线性回归残差图。残差围绕 0 附近分布时，说明模型大致捕捉了主要趋势；如果残差出现明显结构，则需要进一步检查变量遗漏、非线性或空间效应。
...
如果你想跟着连载复现代码、修改示例、下载图件，或者后续系统学习地学 AI 方法，可以先收藏这个项目；如果你希望在自己的研究或教学中继续改造这些示例，也欢迎 fork。后续每一期都会尽量同步保留公众号版、GitHub 长文版、notebook、图件和参考资料。
